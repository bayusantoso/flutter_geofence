    ...
    private fun startGeofencingService(context: Context) {
        synchronized(sServiceStarted) {
            ...
            if (sBackgroundFlutterView == null) {
                ...
                Log.i(TAG, "Ini pluginnya")
                sBackgroundFlutterView = FlutterNativeView(context, true)

                val registry = sBackgroundFlutterView!!.pluginRegistry
                if (sPluginRegistrantCallback == null) {
                    Log.i(TAG, "sPluginRegistrantCallback is null")
                } else {
                    Log.i(TAG, "sPluginRegistrantCallback is not null")
                    if (registry == null) {
                        Log.i(TAG, "registry is null")
                        return
                    } else {
                        Log.i(TAG, "registry is not null")
       ...
       when(call.method) {
            ...
            "GeofencingService.promoteToForeground" -> {
                ...
                if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.P) {
                    mContext.startService(Intent(mContext, IsolateHolderService::class.java))
                } else {
                    mContext.startForegroundService(Intent(mContext, IsolateHolderService::class.java))
                }
                Log.i(TAG, "GeofencingService.promoteToForeground")
            }
